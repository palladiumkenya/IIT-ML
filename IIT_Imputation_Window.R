...
all <- readRDS("iit_1206.rds")

# Rename a few variables for clarity target variable
all <- all %>% rename("most_recent_vl" = most_recent,
...
all <- all %>% 
  filter(!grepl("DEAD|COMPLETE|TRANSFER|DIED|DISCONTINUE", ExitReason)) %>%
  dplyr::select( -PatientID, -PatientResidentWard, -ExitReason)
...
all$recent_late30_rate <- all$missed30_last5 / all$n_visits_lastfive
all$recent_late5_rate <- all$missed5_last5 / all$n_visits_lastfive
all$recent_late1_rate <- all$missed1_last5 / all$n_visits_lastfive
...
for(i in vars_to_impute){
  for(j in 1:nrow(train_to_impute)){
    if(is.na(train_to_impute[j,i]) & is.factor(train_to_impute[, i])){
      x <- c(as.character(all_impute_mice_train[j, paste0(i, ".1")]),
             as.character(all_impute_mice_train[j, paste0(i, ".2")]),
             as.character(all_impute_mice_train[j, paste0(i, ".3")]),
             as.character(all_impute_mice_train[j, paste0(i, ".4")]),
             as.character(all_impute_mice_train[j, paste0(i, ".5")]))
      ux <- unique(x)
...
for(i in vars_to_impute){
  for(j in 1:nrow(val_to_impute_total)){
    if(is.na(val_to_impute_total[j,i]) & is.factor(val_to_impute_total[, i])){
      x <- c(as.character(vals_mice_wide[j, paste0(i, ".1")]),
             as.character(vals_mice_wide[j, paste0(i, ".2")]),
             as.character(vals_mice_wide[j, paste0(i, ".3")]),
             as.character(vals_mice_wide[j, paste0(i, ".4")]),
             as.character(vals_mice_wide[j, paste0(i, ".5")]))
      ux <- unique(x)
...
saveRDS(outlist, "iit_outlist_1206.rds")
